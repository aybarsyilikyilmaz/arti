...
start_comment = "<!-- ============ SCREEN 5 — Onay ============ -->"
end_section = "</section>"

idx = content.find(start_comment)
if idx == -1:
    print("ERROR: Start comment not found!")
    exit(1)

end_idx = content.find(end_section, idx) + len(end_section)

# Reconstructed screen 5 and 6 HTML block
screens_replacement = """<!-- ============ SCREEN 5 — Ödeme alınıyor (Loader) ============ -->
      ...
      <section class="screen" id="screen-6" style="background: #fafafa; display: flex; flex-direction: column;">
        ...
        <div style="padding: 1rem 1.5rem 2rem; background: #fafafa; display: flex; gap: 1rem; width: 100%; box-sizing: border-box;">
          <button onclick="goTo(0)" style="width: 100%; background: #1E8E7D; color: #fff; border: none; padding: 1.15rem 0; border-radius: 999px; font-weight: 700; font-size: 1.05rem; cursor: pointer; white-space: nowrap; transition: transform 0.1s; text-align: center; display: block; box-shadow: 0 4px 12px rgba(30,142,125,0.15);" onmousedown="this.style.transform='scale(0.97)';" onmouseup="this.style.transform='scale(1)';">
            Ana Sayfaya Dön
          </button>
        </div>
      </section>"""

content = content[:idx] + screens_replacement + content[end_idx:]

# Ensure the labels array is updated to include Screen 5 (loading) and 6 (confirm)
